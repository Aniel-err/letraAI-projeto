...
function TurmaDetalhes() {
  ...
  const fetchMinhasRedacoes = useCallback(async () => {
      if (user?.role === 'aluno') {
          try {
              const res = await api.get('/redacoes');
              const enviadas = res.data.filter(r => r.userId === user.id && r.turmaId === Number(id));
              setMinhasRedacoes(enviadas);
          } catch (err) {
  ...
  useEffect(() => { 
      fetchTurmaDetalhes(); 
      fetchMinhasRedacoes();
  }, [fetchTurmaDetalhes, fetchMinhasRedacoes]);
  ...
  const handleAutorizarReenvio = async (redacaoId) => {
      try {
          ...
          const res = await api.get('/redacoes');
          const filtradas = res.data.filter(r => r.userId === alunoRedacoes[0].userId && r.turmaId === Number(id));
  ...
  const handleVerRedacoesAluno = async (alunoId, alunoNome) => {
      try {
          setLoading(true);
          const res = await api.get('/redacoes');
          const filtradas = res.data.filter(r => r.userId === alunoId && r.turmaId === Number(id));
          setAlunoRedacoes(filtradas);
          setAlunoSelecionadoNome(alunoNome);
          setShowAlunoRedacoes(true);
      } catch (err) {
          console.error(err);
          setMsg('Erro ao buscar redações do aluno.');
      } finally {
          setLoading(false);
      }
  };
  ...
  const handleCreateProposta = async (e) => {
      ...
      try {
          ...
          console.error(err);
          alert(err.response?.data?.message || 'Erro ao criar proposta.');
      }
  };
  ...
  const handleUpdateProposta = async (e) => {
      ...
      try {
          ...
          setMsg('Proposta atualizada com sucesso!');
          setShowEditPropostaModal(false);
          fetchTurmaDetalhes();
      } catch (err) {
          console.error(err);
          alert(err.response?.data?.message || 'Erro ao atualizar proposta.');
      }
  };

  const handleDeleteProposta = async (propostaId) => {
      if(!window.confirm("Deseja mesmo apagar esta proposta? As redações ligadas a ela ficarão órfãs.")) return;
      try {
  ...
  const handleDeleteMinhaRedacao = async (redacaoId) => {
      if(!window.confirm("Deseja mesmo excluir sua redação? Você terá que enviar uma nova imagem.")) return;
      try {
          await api.delete(`/redacoes/${redacaoId}`);
          setMsg('Redação excluída! Você já pode enviar uma nova versão.');
          fetchMinhasRedacoes(); 
      } catch {
  ...
  const handleUploadRedacao = async (e) => {
      e.preventDefault();
      if (!file) return alert('Selecione a imagem da redação.');
      ...
      try {
          ...
          if (envioExistente) {
              ...
              setMsg('Redação atualizada com sucesso! A correção anterior foi removida.');
          } else {
              await api.post('/redacoes/upload', formData);
              setMsg('Redação enviada com sucesso!');
          }
          ...
          console.error(err);
          alert(err.response?.data?.message || 'Erro ao processar o envio da redação.'); 
      } finally { 
                                            ...
                                            <>
                                                <Button variant="outline-warning" size="sm" className="fw-bold flex-fill flex-md-grow-0" onClick={() => {
                                                    setPropostaEditData({
                                                        id: p.id,
                                                        titulo: p.titulo,
                                                        textoMotivador: p.textoMotivador,
                                                        prazo: formatForInput(p.prazo)
                                                    });
                    ...
                    <Card.Body className="p-0">
                        ...
                        {aprovados.length > 0 ? (
                            ...
                            <ListGroup.Item key={aluno.id} className="d-flex flex-column flex-sm-row justify-content-between align-items-start align-items-sm-center py-3 gap-2 bg-body text-body">
                                ...
                                <div className="d-flex gap-2 w-100 justify-content-sm-end">
                                    <Button variant="outline-primary" className="flex-fill flex-sm-grow-0 fw-bold" onClick={() => handleVerRedacoesAluno(aluno.id, aluno.nome)}>
                                        👁️ Ver Redações
                                    </Button>
              ...
              {loading ? <div className="text-center"><Spinner animation="border"/></div> : (
                  <ListGroup>
                      {alunoRedacoes.length > 0 ? alunoRedacoes.map(r => (
                          <ListGroup.Item key={r.id} className="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center py-3 gap-3">
                              <div className="w-100">
                                  <strong className="d-block text-body fs-5">{r.Proposta ? r.Proposta.titulo : 'Sem Proposta Vinculada'}</strong>
                              ...
                              <div className="d-flex gap-2">
                                  {r.status === 'Solicitado Reenvio' && <Button variant="success" size="sm" onClick={() => handleAutorizarReenvio(r.id)}>✅ Autorizar Reenvio</Button>}
